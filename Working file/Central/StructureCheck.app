PROC GLOBAL
  ...
  numeric found, ycluster, zcluster, fwrt, e, hhe, bioe, wme, casepart, x, i, j;
...
preproc
  ...
  while loadcase( PEEL ) do
    ...
    {+US}
    if AHRESULT <> 1 then
      yresult = concat( edit("9",AHRESULT), "-Incomplete" )
    elseif casepart then
      yresult = concat( edit("9",AHRESULT), "-In progress" )
    else
      yresult = concat( edit("9",AHRESULT), "-Complete" )
    endif;
    {US+}
    ...
    elseif AHRESULT = 1 then
      ...
      { eligible women }
      x = count( PEEL.MOD_1 where V107 > 0 & V102 = 2 );
      if x <> AHWOMEN then
        getmessage( 006 );
      ...
      { eligible children }
      x = count( PEEL.MOD_1 where V108 > 0 );
      if x <> AHKIDS then
        getmessage( 007 );
      ...
      wmbio = count( PEEL.MOD_1 where V107 <> 0 );
      x = soccurs( MOD_4 );
      if x <> wmbio then
        getmessage( 010 );
	  ...
	  chbio = count( PEEL.MOD_1 where V108 > 0 );
      x = soccurs( MOD_5 );
      if x <> chbio then
        getmessage( 010 );
      ...
      if soccurs(MOD_1)  | soccurs(MOD_1B) | soccurs(MOD_2) | soccurs(MOD_3) |  soccurs(MOD_4_INF) |
         ...
         soccurs(MOD_7_INF) | soccurs(MOD_71) | soccurs(MOD_72) | soccurs(MOD_73) | 
  ...
  if fwrt then writecase( CTRL_CLUSTER, CLUSTER ) endif;
  close( CTRL_CLUSTER );
  { rewrite household sample file if necessary }
  if rwrthha then writecase( SAMPSEL, XCLUSTER ) endif;
  close( SAMPSEL );
