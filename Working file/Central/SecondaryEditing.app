...
{ !! TO DO - expand for country specific context and needs, including all edits/checks that aren't appropriate during CAPI data collection }
  ...
  set explicit; { ensuring this is always active }
 ...
 { --- working variables ------------------------------------------------------------------------ }
  ...
  numeric ismonth, isyear, isday, isage, flag, nEvents, LcmcB, LcmcM, UcmcB, UcmcM;
  numeric b, e, i, j, k, m, mRow, n, t, v, x, y;
  numeric ch, cy, cm, cd, dy, dm, dd, rp, rc, x1, x2, du, dn, di, xy, xm, xd, vd, vy, vm, rw, rh, eno;
  numeric mind, minai, mindb, minaim, minabm, minam, minamm, minau, minaum, minas, minab, mincd, minpd, lmax, lmin;
  numeric maxd, maxai, maxdb, maxaim, maxabm;
...
preproc
  ...
  maxaim   = 95;        { !!! Maximum age at interview - men           }
...
preproc
  ...
  { !!!! uncomment at imputation time }
    ...
    if HHEA <> prevCL then
      loadcase( WEIGHTS, HHEA ); { !!! adjust this file as necessary }
    endif;
    if AHRESULT = 1 then
...
{+MEN}
  endif; } { !! TO DO GD - figure out what weights are needed and where to add them... don't think this is quite right/applicable }
{MEN+}
...
PROC MOD_1_EDT
  ...
  if AHTOTVISITS = 1 then
    if valid(V100H) & valid(V113H) then
      if V100H > V113H then
        errmsg( 0070, "V100", V100H, V100M, "V113", V113H, V113M );
      elseif V100H = V113H then
        if valid(V100M) & valid(V113M) & V100M > V113M then
          errmsg( 0070, "V100", V100H, V100M, "V113", V113H, V113M );
        endif
      endif
    endif
...
{ --------------------------------------------------------------------------- }
  ...
  if count( $ where V103 = 2 {{+POLIG} & V102 = 1 {POLIG+}} ) >= 2  { !! TO DO - uncomment if poligamy in country }
   ...
   | count( $ where V103 = 7 & V102 = 1 ) >= 2
   | count( $ where V103 = 7 & V102 = 2 ) >= 2 then
...
{ --------------------------------------------------------------------------- }
   ...
   { Check read/writing ability }
     if ( V112(i) in 1,2 & ( V109(i) = 2 | V111(i) = 1 ) ) | ( V112(i) in 3,4 & V111(i) > 2 ) then 
       errmsg( 0019, i, V109(i), V111(i), V112(i) );
     endif;
...
PROC MOD_4_EDT
  ...
  { Time of interview } 
    if valid(V400AH(i)) & valid(V431H(i)) then
      if V400AH(i) > V431H(i) then
        errmsg( 0070, concat("V400A(",edit("Z",i),")"), V400AH(i), V400AM(i), concat("V431(",edit("Z",i),")"), V431H(i), V431M(i) );
      elseif V400AH(i) = V431H(i) then
        if valid(V400AM(i)) & valid(V431M(i)) & V400AM(i) > V431M(i) then
          errmsg( 0070, concat("V400A(",edit("Z",i),")"), V400AH(i), V400AM(i), concat("V431(",edit("Z",i),")"), V431H(i), V431M(i) );
        endif
      endif
    endif;
  ...
  { Date of birth of the woman }
    ...
    elseif valid(V402(i)) & !( (V402(i) - V104(V400AD(i))) in (-2):2 ) then
      if run1 then errmsg( 0041, i, V402(i), V400AD(i), V104(V400AD(i)) ) endif;
    endif;
  enddo;
...
PROC ANTHRO_WOMEN_EDT
  ...
  { Time of interview } 
      if valid(V400AAH(i)) & valid(AN400NH(i)) then
        if V400AAH(i) > AN400NH(i) then
          errmsg( 0070, concat("V400AA(",edit("Z",i),")"), V400AAH(i), V400AAM(i), concat("V408(",edit("Z",i),")"), AN400NH(i), AN400NM(i) );
        elseif V400AH(i) = AN400NH(i) then
          if valid(V400AAM(i)) & valid(AN400NM(i)) & V400AAM(i) > AN400NM(i) then
            errmsg( 0070, concat("V400AA(",edit("Z",i),")"), V400AAH(i), V400AAM(i), concat("V408(",edit("Z",i),")"), AN400NH(i), AN400NM(i) );
          endif
        endif
      endif;
  ...
  for i in $ do
    ...
    { weight for women }
    if run1 & !xwt in 20.0:150.0,999.94:999.96,missing then
      errmsg( 0030, "Woman", i, V400AAD, "AN407", xwt, hwage, 2 )
    endif;

    { height for women }
    if run1 & !xht in 100.0:200.0,999.4:999.6,missing then
...
PROC MOD_5_EDT
  ...
  { Time of interview } 
      if valid(V500AH(i)) & valid(V564H(i)) then
        if V500AH(i) > V564H(i) then
          errmsg( 0070, concat("V500(",edit("Z",i),")"), V500AH(i), V500AM(i), concat("V564(",edit("Z",i),")"), V564H(i), V564M(i) );
        elseif V500AH(i) = V564H(i) then
          if valid(V500AM(i)) & valid(V564M(i)) & V500AM(i) > V564M(i) then
            errmsg( 0070, concat("V500(",edit("Z",i),")"), V500AH(i), V500AM(i), concat("V564(",edit("Z",i),")"), V564H(i), V564M(i) );
          endif
        endif
      endif;
  ...
  for i in MOD_5_EDT do
    ...
    if !xwt in missing,notappl,99.94:99.96 then
      ...
      if !GetDecimal( AN518, 2 ) in 0,5 then
        errmsg( 0032, i, V500D, AN518 );
      endif;
    endif;
    ...
    if (di - hwlcmc) < 24 & AN517 = 2 |
       (di - hwucmc) > 23 & AN517 = 1 then
      if !ver100 then errmsg( 0033, i, V500D, AN517, di-hwlcmc ) endif;
      kount(28) = kount(28) + 1;
    endif;
  ...
  { Time of interview } 
      if valid(V500AAH(i)) & valid(V519H(i)) then
        if V500AAH(i) > V519H(i) then
          errmsg( 0070,  concat("V500AA(",edit("Z",i),")"), V500AAH(i), V500AM(i),  concat("V519(",edit("Z",i),")"), V519H(i), V519M(i) );
        elseif V500AAH(i) = V519H(i) then
          if valid(V500AAM(i)) & valid(V519M(i)) & V500AAM(i) > V519M(i) then
...
PROC MOD_61F_EDT
  ...
  { Time of interview } 
      if valid(V6100AH) & valid(V6604H) then
        if V6100AH > V6604H then
          errmsg( 0070, "V6100A", V6100AH, V6100AM, "V6604", V6604H, V6604M );
        elseif V6100AH = V6604H then
          if valid(V6100AM) & valid(V6604M) & V6100AM > V6604M then
            errmsg( 0070, "V6100A", V6100AH, V6100AM, "V6604", V6604H, V6604M );
          endif
        endif
      endif;
...
PROC MOD_66AF_EDT
  ...
  if ( sleep < 3 ) | ( sleep > 12 ) then 
    if run1 | dprev then 
      errmsg( 0071, "A - sleeping and resting - ", sleep );
    endif;
  endif;
  
  { Sleeping/resting is sole activity }
  for i in MOD_66BF do 
    if V6601P_15(i) = "A" & V6601S_15(i) <> "" then 
      errmsg( 0072, V6601H(i), 15, V6601S_15(i)); 
      break;
    endif;
    if V6601P_30(i) = "A" & V6601S_30(i) <> "" then 
      errmsg( 0072, V6601H(i), 30, V6601S_30(i)); 
      break;
    endif;
    if V6601P_45(i) = "A" & V6601S_45(i) <> "" then 
      errmsg( 0072, V6601H(i), 45, V6601S_45(i)); 
      break;
    endif;
    if V6601P_60(i) = "A" & V6601S_60(i) <> "" then 
      errmsg( 0072, V6601H(i), 60, V6601S_60(i)); 
      break;
    endif;
  enddo;
  
  { Sleeping/resting is not a secondary activity }
  for i in MOD_66BF do 
    if V6601S_15(i) = "A" | V6601S_30(i) = "A" | V6601S_45(i) = "A" | V6601S_60(i) = "A" then
      errmsg( 0073, V6601H(i) );
      break;
    endif;
  enddo;
...
PROC MOD_61M_EDT 
  ...
  { Time of interview } 
      if valid(M6100AH) & valid(M6604H) then
        if M6100AH > M6604H then
          errmsg( 0070, "M6100A", M6100AH, M6100AM, "M6604", M6604H, M6604M );
        elseif M6100AH = M6604H then
          if valid(M6100AM) & valid(M6604M) & M6100AM > M6604M then
            errmsg( 0070, "M6100A", M6100AH, M6100AM, "M6604", M6604H, M6604M );
          endif
        endif
      endif;
...
PROC MOD_66AM_EDT
  ...
  { Sleeping/resting is sole activity }
  for i in MOD_66BF do 
    if M6601P_15(i) = "A" & M6601S_15(i) <> "" then 
      errmsg( 0072, M6601H(i), 15, M6601S_15(i)); 
      break;
    endif;
    if M6601P_30(i) = "A" & M6601S_30(i) <> "" then 
      errmsg( 0072, M6601H(i), 30, M6601S_30(i)); 
      break;
    endif;
    if M6601P_45(i) = "A" & M6601S_45(i) <> "" then 
      errmsg( 0072, M6601H(i), 45, M6601S_45(i)); 
      break;
    endif;
    if M6601P_60(i) = "A" & M6601S_60(i) <> "" then 
      errmsg( 0072, M6601H(i), 60, M6601S_60(i)); 
      break;
    endif;
  enddo;
  ...
  { Sleeping/resting is not a secondary activity }
  for i in MOD_66BF do 
    if M6601S_15(i) = "A" | M6601S_30(i) = "A" | M6601S_45(i) = "A" | M6601S_60(i) = "A" then
      errmsg( 0073, M6601H(i) );
      break;
    endif;
  enddo;
