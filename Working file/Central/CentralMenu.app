...
PROC GLOBAL
  ...
  alpha(80)  lookup, report, work, rawdata, final, accepted;
  alpha(80)  entry, central, temp, contrl, peel;
  alpha(15)  xparaux;
  alpha(90)  chkfile;
  alpha(12)  executable;
  alpha(32)  fname;                        { filenames for various files }
...
// preparation for ftp transmission as client 
// write out a sync file as the client
	setfile(syncFile,pathname(application) + SyncFilename,create);

	filewrite(syncFile,"[SyncFile]");
	filewrite(syncFile,"Version=CSPro 6.3");
	filewrite(syncFile,"Description=Download data from field...");

// the RootFolder attribute is optional; it determines what / equates to
// when the client uses the CreateServerPath or ServerPath commands;
// here we'll receive files in a folder called Received, which we first have to create
// the current folder is supervisor and the root folder has to be the main folder
	i = length(pathname(application))-11;
	filewrite(syncFile,"RootFolder=%s",pathname(application)[1:i]); 
	filewrite(syncFile,"[Connection]");
//	filewrite(syncFile,"[Server]");
	filewrite(syncFile,"Type=Dropbox");
//	filewrite(syncFile,"Type=FTP");
//	filewrite(syncFile,"Host=ftp://%s",ftp_server);
//	filewrite(syncFile,"username=%s",ftp_user);
//	filewrite(syncFile,"password=%s",ftp_password);
	filewrite(syncFile,"[Sync]");

// Receive map from server server
	filewrite(syncFile,"ClientPath=/DATA/RAW");
	filewrite(syncFile,"ServerPath=/PEEL/COData/RECEIVED");
	filewrite(syncFile,"Put=A*.dat");
	filewrite(syncFile,"Put=O*.dat");
	filewrite(syncFile,"Put=G*.dat");

	filewrite(syncFile,"ClientPath=/DATA/FIELD");
	filewrite(syncFile,"ServerPath=/PEEL/COData/RECEIVED");
	filewrite(syncFile,"Put=A*.dat");
	filewrite(syncFile,"Put=O*.dat");
	filewrite(syncFile,"Put=G*.dat");
	...
	filewrite(syncFile,"ClientPath=/DATA/RAW");
	filewrite(syncFile,"ServerPath=/PEEL/COData/RECEIVED/NOTES");
	filewrite(syncFile,"Put=A*.not");
	
	close(syncFile);
	close(interv); { !! GD TO DO - do i need this?}
	
	if sync(filename(syncFile)) then
		errmsg(120);
		receive_data = 1;
	else
		errmsg(121);
  ...
  function runpff( CSmod, alpha(80) directory );
    if     CSmod = 1 then executable = "CSBatch.exe"
  ...
  {           2 to check clusters from FINAL directory           }
    ...
    FileWrite( piffile, concat( "PEEL=",  strip(rawdata), "\A", strip(xcluster), ".DAT" ) );
...
//    if whatfile = 1 then
  ...
  {  else
    ...
    FileWrite( piffile, concat( "Listing=",     strip(central), "\",  strip(app), ".lst" ) );
    FileWrite( piffile, concat( "WriteData=",   strip(report),  "\",  strip(app), ".wrt" ) );
  ...
  function FieldChk()
    ...
    FileWrite( piffile, "[Files]" );
    FileWrite( piffile, concat( "Application=", strip(central), "\",  strip(app), ".bch" ) );
    FileWrite( piffile, concat( "InputData=",   strip(report),  "\ALLDATA.DAT" ) );
    FileWrite( piffile, concat( "Listing=",     strip(central), "\",  strip(app), ".lst" ) );
  ...
  function genpff( questype )
    ...
    FileWrite( piffile, "[Files]" );
    FileWrite( piffile, concat( "Application=", strip(peel), "\", strip(app), ".ent" ) );
    ...
    FileWrite( piffile, concat( "Listing=",   strip(temp), "\peel.lst" ) );

    FileWrite( piffile, "[ExternalFiles]" );
    FileWrite( piffile, concat( "CTRL_HH=", strip(contrl), "\CT",xcluster,".dat"));
  ...
  function prepfiles( alpha(32) file1, alpha(32) file2, alpha(32) file3, copyto )
    { Using single quotes instead of double quotes as double quotes are needed in the string generated }
    if copyto = 1 then
      FileConcat( concat( strip(report),  "\", strip(file3) ),     { destination }
  ...
  function collapse()
    FileConcat( concat( strip(report),  "\ALLDATA.DAT" ),      { destination }
...
PreProc
  ...
  wrkprj  = "C:\PEEL";
  peel   = concat( strip(wrkprj), "\PEEL" );    { data entry programs }
  entry   = concat( strip(wrkprj), "\ENTRY" );    { interviewer programs }
  superv  = concat( strip(wrkprj), "\SUPERVISOR" );   { supervisor programs }
  rawdata = concat( strip(wrkprj), "\DATA\RAW" );  { data downloaded from FTP/Dropbox }
  accepted = concat( strip(wrkprj), "\DATA\ACCEPTED" );  { data downloaded from FTP/Dropbox }
  central = concat( strip(wrkprj), "\CENTRAL" );  { central office programs }
  final  = concat( strip(wrkprj), "\DATA\FINAL" );    { to store final data after being edited }
  report  = concat( strip(wrkprj), "\REPORTS" );  { to store all reports }
  lookup  = concat( strip(wrkprj), "\LOOKUP" );      { reference directory for clusters, sample and teams files }
  contrl = concat( strip(wrkprj), "\DATA\CONTROL" );      { folder holding cluster control files for households }
...
PROC WFINAL
  ...
  elseif action = 2 then       { list questionnaires for a cluster }
    if files_in( rawdata ) then 
      if clustopen() then  
        ...
        StructureCheck( 1 );  
        runpff( 1, central );
        prepfiles( "StructureCheck.err", "StructureCheck.wrt", concat( "S", strip(xcluster), ".ids" ), 1 );
        textview( strip(report), concat( "S", strip(xcluster), ".ids" ) );
  ...
  elseif action = 5 then                 { run check field tables }
   ...
   { fname = concat( strip(central), "\", strip(apptoday), ".RTF");
    TheCommand = concat( '"C:\Program Files\Microsoft Office\Office\WINWORD.EXE" ', fname ); { !! TO DO - must change file path of Office !! }
  ...
  elseif action = 6 then                  { status report }
   { filerep = accept( "Type of report desired", "By Cluster", "By Supervisor" ); { !! TO DO GD - look up what kind of status reports I agreed to }
    ...
    makestat( filerep );
    runpff( 1, central );
    textview( strip(report), "StatusReport.WRT" );
    reenter WFINAL;
